	...
	ComPtr<ID3D12Device> pDevice, ComPtr<ID3D12GraphicsCommandList> pCmdList, PrimitiveTiype type)
{
	...
	DXRReflectionApp::gUploadBuffers.push_back(vertexUpload);
	DXRReflectionApp::gUploadBuffers.push_back(indexUpload);
...
void AccelerationStructure::BuiltTopLevelAS(ComPtr<ID3D12Device> pDevice, ComPtr<ID3D12GraphicsCommandList> pCmdList, std::vector<AccelerationStructureBuffers> pBottomLevelAS, uint64_t& tlasSize, AccelerationStructureBuffers& buffers)
{
	...
	transformation[2] = Matrix::CreateTranslation(Vector3(3, 0, 0));
	transformation[3] = Matrix::CreateTranslation(Vector3(6, 0, 0));
	transformation[4] = Matrix::CreateTranslation(Vector3(9, 0, 0));
	transformation[5] = Matrix::CreateTranslation(Vector3(12, 0, 0));
	transformation[6] = Matrix::CreateTranslation(Vector3(15, 0, 0));
