...
)
{
	...
	resource->vertexBuffer->SetName(L"vertexBuffer");
	...
	resource->indexBuffer->SetName(L"indexBuffer");

	vertexUpload->SetName(L"VeretexUpload");
	indexUpload->SetName(L"IndexUpload");

	DXRLightingApp::gUploadBuffers.push_back(vertexUpload);
	DXRLightingApp::gUploadBuffers.push_back(indexUpload);
	return resource;
}
